...
void KDLRobot::getInverseKinematicsVel(KDL::Twist &twist,KDL::JntArray &q_dot)
{
    int res=ikVelSol_->CartToJnt(jntArray_,twist,q_dot);
    if(res < 0) {std::cout << ikVelSol_->strError(res) << std::endl;};
}

void KDLRobot::getInverseKinematicsAcc(KDL::Twist &xe_dot_dot,KDL::JntArray &q_dot_dot)
{
    Eigen::VectorXd qdotdot_vector=pseudoinverse(s_J_ee_.data)*(toEigen(xe_dot_dot)-s_J_dot_ee_.data*jntVel_.data);

    for(unsigned int i=0;i<n_;i++)
    {
        q_dot_dot(i)=qdotdot_vector(i);
    }
}
